...
pick = on_command("捡漂流瓶", aliases={"pick"}, priority=5, block=True)
...
check_ban = on_command("查看漂流瓶黑名单", permission=SUPERUSER,
                       priority=5, block=True)
...
async def _(event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    await throw.finish(Message(Sea(event).throw(msg)), at_sender=True)
